        ...
        suspend fun getStoryDetail(storyId: String): Flow<Result<StoryDetailResponse>> {
            return flow {
                try {
                    ...
                    val authorization = "Bearer $token"
                    val response = apiService.getStoryDetail(authorization, storyId)
                    emit(Result.Success(response))
                } catch (e: Exception) {
        ...
        suspend fun uploadStory(
            ...
            return flow {
                ...
                try {
                    ...
                    val authorization = "Bearer $token"
                    val response = apiService.uploadStory(authorization, file, descriptionPart, lat, lon)

                    if (response.error == false) {
                        emit(Result.Success(response))
                    } else {
                        emit(Result.Error(response.message ?: "Upload cerita gagal"))
                    }
                } catch (e: Exception) {
        ...
        suspend fun getStoriesWithLocation(): Flow<Result<StoryResponse>> {
            return flow {
                try {
                    ...
                    val authorization = "Bearer $token"
                    val response = apiService.getStoriesWithLocation(authorization, location = 1)
